...
class TicTacToe(Game):
    ...
    def result(self, board, square1):
        """Place a marker for current player on square."""
        player = board.to_move
        board = board.new({square1: player, square1: player}, to_move=('O' if player == 'X' else 'X'))
        win = k_in_row(board, player, square1, self.k)
...
play_game(TicTacToe(height=4, width=4, k=4), dict(X=player(minimax_search), O=random_player), verbose=True)
